...
// //axios 1
// try {
//   const res = await axios.post("https://the-book-haven-server-rose.vercel.app/comments", newComment, {
...
// //axios 2
//     try {
//   const res = await axios.post("https://the-book-haven-server-rose.vercel.app/reviews", newReview, {
//     headers: { "Content-Type": "application/json" },
//   });
...
const BookDetails = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#f0f8ff] to-white dark:from-gray-900 dark:to-gray-800 p-4 sm:p-6">
      <Toaster />
      <div className="max-w-4xl mx-auto bg-white dark:bg-gray-800 rounded-2xl shadow-lg overflow-hidden grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Book Cover */}
        <div className="flex justify-center items-center bg-[#e8f9ff] dark:bg-gray-700 p-4 md:p-6">
          <img
            src={coverImage}
            alt={title}
            className="rounded-xl w-48 h-64 sm:w-56 sm:h-72 md:w-64 md:h-80 object-cover shadow-md hover:scale-105 transition-transform duration-300"
        ...
        {/* Book Info */}
        <div className="p-4 sm:p-6 flex flex-col justify-center space-y-3 sm:space-y-4 text-gray-800 dark:text-gray-200">
          <h2 className="text-2xl sm:text-3xl font-bold text-[#0abde3]">{title}</h2>
      ...
      {/* Comments Section */}
      <div className="max-w-4xl mx-auto mt-8 p-4 sm:p-6 bg-white dark:bg-gray-800 rounded-xl shadow-md text-gray-800 dark:text-gray-200">
        <h3 className="text-xl font-bold text-[#0abde3] mb-4">Comments</h3>
        ...
        {user ? (
          <form onSubmit={handleAddComment} className="flex flex-col sm:flex-row gap-2 mb-4">
            <input
              ...
              onChange={(e) => setCommentText(e.target.value)}
              className="flex-1 border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#0abde3] 
                         bg-white text-gray-700 dark:bg-gray-800 dark:text-gray-100 dark:border-gray-600"
            />
            <button
              type="submit"
              className="bg-[#0abde3] text-white rounded-md px-4 py-2 hover:bg-[#08a4d6] transition-colors"
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-4">Please log in to add a comment.</p>
        )}
        ...
        {loadingComments ? (
          ...
          <div className="space-y-4 max-h-80 overflow-y-auto">
            {comments.map((c, index) => (
              <div key={index} className="flex items-start gap-3 bg-gray-50 dark:bg-gray-700 p-3 rounded-lg shadow-sm">
                {c.userPhoto ? (
                  <img src={c.userPhoto} alt={c.userName} className="w-10 h-10 rounded-full object-cover" />
                ) : (
                ...
                <div>
                  <p className="font-semibold text-[#0abde3] dark:text-[#66d9ef]">{c.userName}</p>
                  <p className="text-gray-700 dark:text-gray-200">{c.comment}</p>
                  <p className="text-xs text-gray-400 dark:text-gray-400 mt-1">{new Date(c.createdAt).toLocaleString()}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="max-w-4xl mx-auto mt-8 p-4 sm:p-6 bg-white dark:bg-gray-800 rounded-xl shadow-md text-gray-800 dark:text-gray-200">
        ...
        {user ? (
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-4">Please log in to add a review.</p>
        )}
          ...
          <div className="space-y-4 max-h-80 overflow-y-auto">
            {reviews.map((r, index) => (
              <div key={index} className="flex items-start gap-3 bg-gray-50 dark:bg-gray-700 p-3 rounded-lg shadow-sm">
                ...
                <div>
                  <p className="font-semibold text-[#0abde3] dark:text-[#66d9ef]">{r.userName}</p>
                  <p className="text-gray-700 dark:text-gray-200">{r.review}</p>
                  <p className="text-xs text-gray-400 dark:text-gray-400 mt-1">{new Date(r.createdAt).toLocaleString()}</p>
                </div>
              </div>
            ))}
          </div>
